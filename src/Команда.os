...
Процедура УстановитьОбработчикОжидания(Обработчик, Знач Интервал = 5000) Экспорт
	
	ОбработчикОжиданияПроцесса = Новый Структура;
	ОбработчикОжиданияПроцесса.Вставить("Обработчик", Обработчик);
	ОбработчикОжиданияПроцесса.Вставить("Интервал", Интервал);
...
Процедура ПолучитьВыводПокаПроцессИсполняется(Процесс)
	...
	ПериодОпроса = ОбработчикОжиданияПроцесса.Период;
	...
	Пока НЕ Процесс.Завершен ИЛИ Процесс.ПотокВывода.ЕстьДанные ИЛИ Процесс.ПотокОшибок.ЕстьДанные Цикл
		...
		Если УстановленОбработчикОжиданияПроцесса Тогда
			...
			НодоВызватьОбработчик = ДатаНачала + ПериодОпроса * 1000 
									<= ТекущаяУниверсальнаяДатаВМиллисекундах();
			Если НодоВызватьОбработчик Тогда
				
				Отказ = Ложь;
				Лог.Отладка("Вызов обработчика ожидания");
				Попытка
					ОбработчикОжиданияПроцесса.Обработчик.ОбработкаОжиданияПроцесса(Процесс, Отказ, ПериодОпроса);
				Исключение
