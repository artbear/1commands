...
Процедура УстановитьПравильныйКодВозврата(Знач ПарамОжидаемыйКодВозврата) Экспорт
	...
	ОжидаемыйКодВозврата = ПарамОжидаемыйКодВозврата;
КонецПроцедуры
...
Функция ЗапуститьИПодождать()
	...
	Если ПроверяемКодВозврата И КодВозврата <> ОжидаемыйКодВозврата Тогда
		ТекстОшибки = СтрШаблон("Ожидали, что код возврата <%1> команды <%2> совпадет с ожидаемым <%3>, а это не так", 
			КодВозврата, Приложение, ОжидаемыйКодВозврата);
		Ожидаем.Что(КодВозврата, ТекстОшибки).Равно(ОжидаемыйКодВозврата);
	КонецЕсли;
